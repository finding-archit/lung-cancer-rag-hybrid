...
JSON Response:"""
    ...
    def graph_search_with_traversal(self, entity, intent, top_k=3, max_path_length=3):
        ...
        """
        print(f"\n[2. GRAPH SEARCH - Entity-Intent Traversal]")
        ...
        with self.neo4j_driver.session() as session:
            ...
            entity_result = session.run("""
                MATCH (s:Section)-[:CONTAINS_ENTITY]->(e:Entity)
                WHERE any(keyword IN $keywords WHERE toLower(e.value) CONTAINS toLower(keyword))
                WITH s, 
                     COLLECT(DISTINCT {type: e.type, value: e.value}) as matched_entities
                RETURN s.name as section_name, 
                       s.text as text, 
                       matched_entities
            ...
            for record in entity_result:
                ...
                # Entity match count
                entity_matches = sum(
                    1 for e in matched_entities
                    if any(keyword.lower() in e["value"].lower() for keyword in entity_keywords)
                )
                # Intent match count
                intent_matches = sum(
                    1 for e in matched_entities
                    if entity_type.lower() in e["type"].lower() or intent.lower() in e["type"].lower()
                )
                score = min(entity_matches / 3.0, 1.0)
            ...
            if not entity_sections or len(entity_sections) == 0:
                print("  No entity matches found")
                return []

            print(f"  Found {len(entity_sections)} sections matching entity '{entity}'")
            print(f"  Top section: {entity_sections[0]['section_name']} (score: {entity_sections[0]['entity_score']:.4f}, intent matches: {entity_sections[0]['intent_matches']})")

            # Step 2: Traverse for intent-based sections
            traversal_result = session.run("""
                MATCH path = (start:Section {name: $start_section})-[*1..3]-(end:Section)
                WHERE end <> start
                AND EXISTS {
                    MATCH (end)-[:CONTAINS_ENTITY]->(e:Entity)
                    WHERE toLower(e.type) CONTAINS toLower($entity_type) OR toLower(e.type) CONTAINS toLower($intent)
                }
                WITH end, 
                     length(path) as path_length
                MATCH (end)-[:CONTAINS_ENTITY]->(e:Entity)
                WHERE toLower(e.type) CONTAINS toLower($entity_type) OR toLower(e.type) CONTAINS toLower($intent)
                WITH end.name as section_name,
                     end.text as text,
                     MIN(path_length) as min_path_length,
                     COUNT(DISTINCT e) as intent_matches,
                     COLLECT(DISTINCT {type: e.type, value: e.value}) as matched_entities
                RETURN section_name, 
                       text, 
                       min_path_length,
                       intent_matches,
                       matched_entities
                ORDER BY min_path_length ASC, intent_matches DESC
                LIMIT $limit
            """, start_section=entity_sections[0]['section_name'], entity_type=entity_type, intent=intent, limit=top_k - 1)

            traversal_sections = []
            for record in traversal_result:
                if record['text']:
                    path_weight = 1.0 / record['min_path_length']
                    intent_score = min(record['intent_matches'] / 3.0, 1.0)
                    graph_score = path_weight * intent_score

                    traversal_sections.append({
                        'section_name': record['section_name'],
                        'text': record['text'],
                        'graph_score': graph_score,
                        'path_length': record['min_path_length'],
                        'intent_matches': record['intent_matches'],
                        'matched_entities': record['matched_entities']
                    })

            # Combine direct entity match and traversal results for top_k
            final_sections = []

            # Best entity section (direct match) now intent_matches is counted
            best_entity_section = {
                'section_name': entity_sections[0]['section_name'],
                'text': entity_sections[0]['text'],
                'graph_score': entity_sections[0]['entity_score'],
                'path_length': 0,
                'intent_matches': entity_sections[0]['intent_matches'],
                'matched_entities': entity_sections[0].get('matched_entities', [])
            }
            final_sections.append(best_entity_section)

            for section in traversal_sections:
                if section['section_name'] != best_entity_section['section_name']:
                    final_sections.append(section)
                if len(final_sections) >= top_k:
                    break

            # Fill remaining slots with other entity sections if needed
            while len(final_sections) < top_k and len(entity_sections) > len(final_sections):
                next_section = entity_sections[len(final_sections)]
                if next_section['section_name'] not in [s['section_name'] for s in final_sections]:
                    final_sections.append({
                        'section_name': next_section['section_name'],
                        'text': next_section['text'],
                        'graph_score': next_section['entity_score'] * 0.5,
                        'path_length': 0,
                        'intent_matches': next_section['intent_matches'],
                        'matched_entities': next_section.get('matched_entities', [])
                    })
    ...
    def final_ranking(self, normalized_sections, top_k=3):
        ...
        print(f"\n[4. FINAL RANKING - Hybrid Scores]")

        for section in normalized_sections.values():
            section['hybrid_score'] = section['vector_score'] + section['graph_score']

        final_ranked = sorted(normalized_sections.values(), 
                            key=lambda x: x['hybrid_score'], 
                            reverse=True)[:top_k]

        print(f"  Top {top_k} sections by hybrid score:")
        for i, r in enumerate(final_ranked, 1):
            print(f"    {i}. {r['section_name']}: {r['hybrid_score']:.4f} "
                  f"(vector: {r['vector_score']:.4f}, graph: {r['graph_score']:.4f})")
...
if __name__ == "__main__":
    ...
    JSON_FILE = "lung_cancer_fixed_FOR_RAG.json"  # <-- Use your newly converted file
